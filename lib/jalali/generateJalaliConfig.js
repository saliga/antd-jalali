...
var parseNoMatchNotice = function () {
    ...
    (0, warning_1.noteOnce)(false, "Not match any format. Please help to fire a issue about this.");
};
var generateJalaliConfig = {
    // get
    getNow: function () { return (0, dayjs_1.default)(); },
    getFixedDate: function (string) { return (0, dayjs_1.default)(string, "YYYY-MM-DD"); },
    getEndDate: function (date) { return date.endOf("month"); },
    getWeekDay: function (date) {
        if (!(date === null || date === void 0 ? void 0 : date.weekday())) {
    ...
    getMillisecond: function (date) { return date.millisecond(); },
    setMillisecond: function (date, second) { return date.millisecond(second); },
    // Compare
    isAfter: function (date1, date2) { return date1.isAfter(date2); },
    isValidate: function (date) { return date.isValid(); },
    locale: {
        getWeekFirstDate: function (locale, date) { return date.locale(parseLocale(locale)).weekday(0); },
        getWeekFirstDay: function (locale) { return (0, dayjs_1.default)().locale(parseLocale(locale)).localeData().firstDayOfWeek(); },
        getWeek: function (locale, date) { return date.locale(parseLocale(locale)).week(); },
        getShortWeekDays: function (locale) { return (0, dayjs_1.default)().locale(parseLocale(locale)).localeData().weekdaysMin(); },
        getShortMonths: function (locale) { return (0, dayjs_1.default)().locale(parseLocale(locale)).localeData().monthsShort(); },
        ...
        parse: function (locale, text, formats) {
            ...
            for (var i = 0; i < formats.length; i += 1) {
                ...
                if (format.includes("wo") || format.includes("Wo")) {
                    ...
                    var firstWeek = (0, dayjs_1.default)(year, "YYYY").startOf("year").locale(localeStr);
                ...
                var date = (0, dayjs_1.default)(formatText, {
                    format: format,
                    locale: "fa_IR",
                    //  @ts-ignore
                    jalali: true,
